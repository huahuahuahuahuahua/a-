	...
	<ezpage :title="shelfInfo.name">
		<view slot="contentSection" style="height: 100%;">
			<canvas
				...
				canvas-id="myPoster"
				type="2d"
				style="position: absolute;left:-350px;width:350px;height:750px;"
			></canvas>
			...
			<view style="justify-content: start;display: flex;flex-wrap: wrap;padding-bottom: 70px;">
				<view v-for="item in books" style="width:33.3%;padding:10px;margin-bottom: 10px;">
					...
					<view
						...
						@click="btnDeleteBook"
						:data-id="item._id"
						style="font-size:10px;text-align:center;margin-top: 10px;"
					>
						<text style="padding:5px 10px;background:#f3433e;color:#fff;border-radius:5px;">编辑</text>
					</view>
				</view>
			</view>
		</view>
		<view v-if="shelfInfo.isowner" slot="tabSection" style="padding:10px;">
			<view
				v-if="!isEditing"
				style="background-color: #2979ff;width: 100%;height: 50px;border-radius: 25px;display: flex;line-height: 50px;color:#fff;justify-content: space-between;"
			>
				<view
				...
				<view
					@click="btnEnterEditing"
					class="iconfont icon-apps"
					style="width:50px;height: 50px; text-align: center;"
				></view>
			</view>
			<view
				v-else
				style="background-color: #000;width: 100%;height: 50px;border-radius: 25px;display: flex;line-height: 50px;color:#fff;justify-content: space-between;"
			>
				<view></view>
		...
		btnScan() {
			...
			uni.scanCode({
				success: async res => {
					...
					const booksdata = await cloudApi.call({
						name: 'books',
						data: {
							action: 'add',
							shelfid: this.shelfid,
